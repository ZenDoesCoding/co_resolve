    ...
    try:
        ...
        try:
            ...
            for _ in range(max_invocations):
                ...
                try:
                    use_stream = settings.llm_provider != "google"
                        ...
                        async for chunk in response:
                            ...
                            if delta.tool_calls:
                                for tc_chunk in delta.tool_calls:
                                    ...
                                    # Preserve thought signature or extra attributes
                                    extra = {}
                                    if hasattr(tc_chunk, 'model_extra') and tc_chunk.model_extra:
                                        extra.update(tc_chunk.model_extra)
                                    for attr in ('signature', 'thought_signature'):
                                        val = getattr(tc_chunk, attr, None) or (tc_chunk.get(attr) if isinstance(tc_chunk, dict) else None)
                                        if val:
                                            extra[attr] = val
                                    if extra:
                                        if "extra" not in tool_calls_dict[idx]:
                                            tool_calls_dict[idx]["extra"] = {}
                                        tool_calls_dict[idx]["extra"].update(extra)
