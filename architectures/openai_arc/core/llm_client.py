...
class LLMClient:
    def __init__(self):
        if settings.llm_provider == "google" or (settings.openai_base_url and "generativelanguage.googleapis.com" in settings.openai_base_url):
    ...
    async def generate(self, messages, tools=None, stream=False):
        ...
        # Google AI Studio OpenAI compatibility layer requires at least one user message
        ...
        extra_body = {}
        if settings.llm_provider == "google":
            extra_body["reasoning_effort"] = settings.thinking_level
            
        if settings.llm_provider != "google" and settings.openai_base_url and "openrouter.ai" in settings.openai_base_url:
        ...
        if yaml_config["agent"].get("reasoning_mode", True) and settings.llm_provider != "google":
